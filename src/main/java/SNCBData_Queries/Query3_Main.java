...
public class Query3_Main {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            ...
            props.put(StreamsConfig.APPLICATION_ID_CONFIG, "query3_SNCB");
        ...
        public Processor<Windowed<String>, Object, String, String> get() {
            return new Processor<Windowed<String>, Object, String, String>() {
                ...
                public void process(Record<Windowed<String>, Object> record) {
                    ...
                    for (String r : rows) sorted.add(SNCBData.fromCsv(r));
                    sorted.sort(Comparator.comparingLong(SNCBData::getTimestamp));

                    if (sorted.isEmpty()) return;

                    // Build the sequence literal: {POINT(lon lat)@ts, ...}
                    ...
                    // Parse the sequence into a native MEOS tgeogpoint pointer.
                    ...
                    // Serialize the MEOS pointer back to a human-readable WKT string.
                    ...
                    String output = String.format(
                            "[TRAJ][Q3] DeviceID=%-12s | points=%3d | window [%s - %s]%n | trajectory: %s",
                            device_id,
                            sorted.size(),
        ...
        public Processor<Windowed<String>, Object, String, String> get() {
            return new Processor<Windowed<String>, Object, String, String>() {
                ...
                public void process(Record<Windowed<String>, Object> record) {
                    ...
                    for (SNCBData sncbData : sorted) {
                        String wkt   = String.format("POINT(%f %f)@%s",
                                sncbData.getLon(), sncbData.getLat(), millisToTimestamp(sncbData.getTimestamp()));
