...
public class Query7_Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            props.put(StreamsConfig.APPLICATION_ID_CONFIG, "query7_SNCB");
                ...
                public void process(org.apache.kafka.streams.processor.api.Record<String, Object> record) {
                    ...
                    for (int i = 0; i < lefts.size(); i++) {
                        ...
                        for (int j = 0; j < rights.size(); j++) {
                            ...
                            // Keep only the minimum distance per unique (DeviceID_1, DeviceID_2) pair.
                    ...
                    for (int rank = 0; rank < emitCount; rank++) {
                        ...
                        String result = String.format(
                                "[TOPK][Q7] rank=%2d/%d | DeviceID1=%-12d (lon=%9.5f lat=%8.5f)"
                                        + " | DeviceID2=%-12d (lon=%9.5f lat=%8.5f)"
                                        + " | mindist=%10.3f m",
                                rank + 1, emitCount,
                                (long) p[0], p[3], p[4],
                                (long) p[1], p[5], p[6],
                        ...
                        String deviceIDs = String.valueOf(p[0])+String.valueOf(p[1]);
                        log.info(result);
                        context.forward(new Record<>(deviceIDs, result, record.timestamp()));
                    }
