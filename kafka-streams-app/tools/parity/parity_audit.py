#!/usr/bin/env python3
"""Symbol-level parity audit for the MobilityKafka generated MEOS facade.

Measures, per type family, how much of the MEOS public-header function surface
the generated facade (org.mobilitydb.kafka.meos.MeosOps*) exposes, cross-checked
...
Usage:
    python3 tools/parity/parity_audit.py \
        ...
        --jar jar/JMEOS.jar \
        --facade src/main/java/org/mobilitydb/kafka/meos \
...
def main():
    ...
    ap.add_argument("--facade", default=os.path.join(here, "src/main/java/org/mobilitydb/kafka/meos"))
    ...
    L = []
    L.append("# MobilityKafka parity status — MEOS surface audit\n")
    L.append(f"Generated {date.today().isoformat()} by `tools/parity/parity_audit.py`.\n")
    L.append("The MobilityKafka MEOS facade (`org.mobilitydb.kafka.meos.MeosOps*`) exposes "
    ...
    L.append("Per-family runtime behaviour is asserted by "
             "`src/test/java/org/mobilitydb/kafka/meos/MeosFacadeSmokeTest.java`, which constructs "
